...
namespace COM3D2.ModelExportMMD.COM3D2.ModelExportMMD
{
    public class PmxExporter : IExporter
    {
        ...
        public ModelFormat ExportFormat
        {
            get { return ModelFormat.Pmx; }
        }
        ...
        private void ConvertBoneWeight(PmxVertex.BoneWeight[] weights, BoneWeight unityWeight, Transform[] bones)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    Debug.LogError(
                        $"{i}: weight={weightsU[i]} idx={indices[i]} name={names[i]} contains={bonesMap.ContainsKey(names[i])}");
                }
        ...
        private void CreateMeshList(SkinnedMeshRenderer meshRender)
        {
            ...
            if (!vertexIndexMap.ContainsKey(gameObject.transform.parent.gameObject.name))
            {
            ...
            for (int i = 0; i < boneList.Count; i++)
            {
                ...
                for (Transform parent = bone.parent; parent != null; parent = parent.parent)
                {
                    if (bonesMap.ContainsKey(parent.name))
                    {
                        ...
                        {
                            Debug.Log(
                                $"Warning: bone {bone.name} was parented to {boneList[boneParent[i]].name} but was also found parented to {parent.name}");
                        }
        ...
        private void CreateMaterial(Material material, int count)
        {
            ...
            pmxMaterial.Flags = (PmxMaterial.MaterialFlags.DrawBoth | PmxMaterial.MaterialFlags.Shadow |
                                 PmxMaterial.MaterialFlags.SelfShadowMap | PmxMaterial.MaterialFlags.SelfShadow);
        ...
        private void CreateMorphs(TBodySkin skin)
        {
            ...
            for (int j = 0; j < skin.morph.BlendDatas.Count; j++)
            {
                ...
                if (blendData != null)
                {
                    ...
                    {
                        PmxVertexMorph pmxVertexMorph = new PmxVertexMorph(blendData.v_index[k] + vertexBase,
                            new PmxLib.Vector3(-blendData.vert[k].x, blendData.vert[k].z, blendData.vert[k].y));
